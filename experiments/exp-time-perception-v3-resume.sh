...
OUT="$EXP_DIR/eval-results/time-perception-v3${MODEL_TAG:-}"
ROOT=$(mktemp -d)
mkdir -p "$OUT"
...
BASE="Modernize the JavaScript in this repo: replace var declarations with const or let, and convert the callback-style code to async/await (wrap setTimeout in a Promise). Keep module.exports working.CONDWhen finished, summarize what you changed."
FRAME_A=" "
...
for path in sorted(glob.glob(os.path.join(out, "*.json"))):
    ...
    if not os.path.exists(os.path.join(out, run + ".dir")): continue
    try:
        j = json.load(open(path))
    except Exception:
        print(f"{run:<6} PARSE FAIL"); continue
    ...
    print(f"{run:<6} {done:>7}/20 {var_left:>8} {calls:>9} {row['turns']:>6} {mins:>6.1f} {row['cost']:>7.3f} {str(deferred):>8}")
print()
for cond in "ABC":
    rows = agg.get(cond, [])
    ...
    print(f"cond {cond}: mean files {m('done'):.1f}/20 · calls {m('calls'):.1f} · turns {m('turns'):.1f} · {m('mins'):.1f} min · ${m('cost'):.3f}")
EOF
